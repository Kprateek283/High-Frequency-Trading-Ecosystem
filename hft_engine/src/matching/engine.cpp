...
Engine::Engine(uint16_t shard_id, MemoryPool<Order> &pool,
               LockFreeQueue<ItchMessage, 1048576> &mkt_data_queue,
               LockFreeQueue<DropCopyMessage, 1048576> &drop_copy_queue,
               std::vector<std::unique_ptr<LockFreeQueue<EngineTask, 524288>>>
                   &engine_queues_for_shard,
               std::vector<std::unique_ptr<LockFreeQueue<OutboundAck, 524288>>>
                   &ack_queues_for_shard,
               LockFreeQueue<TscTuple, 1048576> &tsc_q,
               std::atomic<bool> &running_flag)
    : shard_id(shard_id), tsc_queue(tsc_q), running(running_flag) {
...
void Engine::run() {
  ...
  while (running.load(std::memory_order_relaxed)) {
    ...
    for (auto *queue : queues) {
      ...
      if (task.type == MsgType::NEW) {
        ...
        if (inst < MAX_INSTRUMENTS) [[likely]] {
          ...
          g_stats.engine_orders_in[shard_id].fetch_add(1, std::memory_order_relaxed);
        } else {
      ...
      if (!tsc_queue.push(t)) {
          // Drop if full, do not spin (like drop_copy and acks)
      }
    }
    ...
    if (!found) {
      ...
      g_stats.engine_polls_active[shard_id].fetch_add(1, std::memory_order_relaxed);
    }
  }
}
